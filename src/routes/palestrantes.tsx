  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="py-16">
        {formatedDataPalestrantes?.length == 0 ? (
          <div className="px-5">
            <EmptyState
              type="no-users"
              variant="card"
          ...
          <div className="mx-auto max-w-full px-10 sm:px-6 lg:px-8">
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {formatedDataPalestrantes?.map((palestrante, index) => (
                  ...
                  <CardContent className="p-6">
                    <div className="flex flex-col items-center text-center">
                      <div className="w-24 h-24 rounded-full bg-primary/10 flex items-center justify-center mb-4 group-hover:bg-primary/15 transition-colors">
                        <img src={palestrante.fields.url_foto} alt={palestrante.fields.nome} className="rounded-full object-cover size-full" />
                      </div>

                      <Badge variant="secondary" className="mb-3 bg-secondary/10 text-secondary hover:bg-secondary/20">
                        {palestrante.fields.empresa_faculdade}
                      </Badge>

                      <h3 className="font-semibold text-foreground mb-1">{palestrante.fields.nome}</h3>
                      <p className="text-sm text-muted-foreground mb-3">{palestrante.fields.empresa_faculdade}</p>

                      <p className="text-sm text-muted-foreground leading-relaxed mb-4 text-justify">{palestrante.fields.descricao}</p>
                    </div>

                    <div className="flex items-center justify-center gap-3">
                      <Link
                        to={palestrante.fields.linkedin}
                        ...
                        aria-label={`LinkedIn de ${palestrante.fields.nome}`}
                      >
                        <Linkedin className="h-4 w-4" />
                      </Link>

                      <Link
                        to={palestrante.fields.instagram}
                        className="w-8 h-8 rounded-full bg-muted flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-colors"
                        aria-label={`LinkedIn de ${palestrante.fields.nome}`}
                      >
